...
\scriptsize
\textit{Notes:} PushT full projected CEM uses 100 pairs $\times$ 3 seeds; PushT re-rank-only uses 100 pairs $\times$ 1 seed; Cube full projected CEM uses 50 pairs $\times$ 3 seeds; Cube re-rank-only uses 100 pairs $\times$ 1 seed. Bootstrap 95\% CIs for $\Delta_{\text{CEM}}$ are reported in Appendix~\ref{app:reproducibility}.
\end{minipage}
\end{table}
...
Whether the ranking gap translates to planning failure depends on pool success mass: in Cube (Case~B), high success mass renders ranking less critical, while in PushT hard subsets (Case~C), low success mass makes ranking essential. Cube therefore provides cross-environment evidence for the endpoint-pool ranking gap, but not for cost-driven planning failure.
...
Figure~\ref{fig:v3_heatmap} visualizes this decomposition across all five pair subsets. Overall, $\Rpool(\Cmodel)=0.084$ with 95\% bootstrap CI $[0.037,0.131]$, $\Rpool(C_\mathrm{V3})=0.258$ with CI $[0.146,0.365]$, and $\Rpool(C_\mathrm{V1})=0.666$ with CI $[0.595,0.735]$. The figure rules out pure CEM convergence compression: $C_\mathrm{V1}$ retains substantial order inside the same pools. It also shows that V1 alone was not the whole story. $C_\mathrm{V3}$ improves over $\Cmodel$, so predictor rollout error contributes to local rank degradation, but $C_\mathrm{V3}$ remains far below $C_\mathrm{V1}$, so actual-terminal encoder geometry and Euclidean cost shape remain limiting. The same pattern holds in the hard subsets. In the 16 invisible-quadrant pairs, $\Rpool(\Cmodel)=0.022$, $\Rpool(C_\mathrm{V3})=0.348$, and $\Rpool(C_\mathrm{V1})=0.775$. In the 21 sign-reversal pairs, $C_\mathrm{V3}$ does not recover ranking ($-0.125$) while $C_\mathrm{V1}$ remains high (0.925). Thus the mechanism is mixed: predictor rollout worsens local ranking, while learned terminal-latent geometry remains weak on the same pool. Panels~\ref{fig:decoupling_evidence}a--c show the endpoint-planning decoupling on a representative pair, while panel~\ref{fig:decoupling_evidence}d extends the V1 attribution to all 100 pairs. This decomposition motivates the calibration experiment in Section~\ref{sec:warp_ensemble}, which tests whether pool-level ranking supervision can correct the local geometry mismatch.

A matched-distribution sanity check gives the same conclusion. On the same 100 PushT pools, restricting pairwise comparisons to candidates whose physical costs differ by more than 20 units leaves $\Cmodel$ near chance (56.4\% correct; random 50.5\%), while $C_\mathrm{V1}$ is nearly perfect (98.6\%). NDCG also separates the rankers: at 10 candidates, $\Cmodel$ reaches 0.630, random ranking 0.610, and $C_\mathrm{V1}$ 0.862. Finally, when the Track~A endpoint artifact is range-restricted to each pool's physical-cost interval, the $m=64$ endpoint correlation drops from 0.473 to 0.189, but remains above $\Rpool(\Cmodel)=0.084$; range restriction contributes to the gap, but it does not explain the large-gap misorderings inside the pools.
