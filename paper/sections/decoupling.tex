...
In Cube, the near-zero $\Rpool$ coexists with relatively high planning success, consistent with Case B: the pool contains sufficient success mass that ranking is less critical for action selection. Cube therefore provides cross-environment evidence for the endpoint-pool ranking gap, not for cost-driven planning failure.
...
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{lccL{0.42\linewidth}}
\toprule
Cost ranker & $\Rpool$ mean$\pm$std & 95\% CI & What it tests \\
\midrule
$\Cmodel$ (predicted latent) & \textbf{0.084}$\pm$0.243 & [0.037, 0.131] & End-to-end predictor, encoder, and latent cost shape \\
$C_\mathrm{V3}$ (actual terminal latent) & 0.258$\pm$0.566 & [0.146, 0.365] & Encoder and latent cost shape after removing predictor rollout \\
$C_\mathrm{V1}$ (physical hinge) & \textbf{0.666}$\pm$0.357 & [0.595, 0.735] & Physical ground-truth ranker on the same pool \\
\bottomrule
\end{tabular}
}
\caption{Same-pool mechanism attribution on 100 PushT default CEM final pools. Intervals are 10{,}000-sample percentile bootstraps over pairs; flat V1 pools are counted as zero correlation, matching the Phase B convention.}
\label{tab:v3-decomposition}
\end{table}

Table~\ref{tab:v3-decomposition} rules out pure CEM convergence compression: $C_\mathrm{V1}$ retains substantial order inside the same pools. It also shows that V1 alone was not the whole story. $C_\mathrm{V3}$ improves over $\Cmodel$, so predictor rollout error contributes to the collapse, but $C_\mathrm{V3}$ remains far below $C_\mathrm{V1}$, so actual-terminal encoder geometry and Euclidean cost shape remain limiting. The same pattern holds in the hard subsets. In the 16 invisible-quadrant pairs, $\Rpool(\Cmodel)=0.022$, $\Rpool(C_\mathrm{V3})=0.348$, and $\Rpool(C_\mathrm{V1})=0.775$. In the 21 sign-reversal pairs, $C_\mathrm{V3}$ does not recover ranking ($-0.125$) while $C_\mathrm{V1}$ remains high (0.925). Thus the mechanism is mixed: predictor rollout worsens local ranking, but learned terminal-latent geometry still fails on the same pool. Panels~\ref{fig:decoupling_evidence}a--c show the endpoint-planning decoupling on a representative pair, while panel~\ref{fig:decoupling_evidence}d extends the V1 attribution to all 100 pairs.
