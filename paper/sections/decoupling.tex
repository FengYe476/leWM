...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/figure3_per_subset.pdf}
\caption{Per-subset rank-1 planning success ($M_\mathrm{rank1}$) at projection dimension $m=64$ under two PushT protocols. Dashed lines show aggregate success. \textbf{(a)}~Full projected CEM: ordinary pairs reach 86.7\% while invisible-quadrant and sign-reversal pairs stay below 5\%. \textbf{(b)}~Re-rank-only: ordinary pairs drop to 55.3\% while latent-favorable pairs rise to 75.0\%. The aggregate masks sharply different subset behavior under both protocols.}
