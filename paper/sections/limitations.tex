...
The evidence has bounded scope. While the primary audit targets LeWM, we apply the core diagnostic to DINO-WM (Section~\ref{sec:dinowm}) and observe pool-level cost collapse in an architecturally distinct planner on PushT. Because DINO-WM's endpoint ranking is also near zero, this is a cross-architecture pool-collapse check rather than a clean replication of LeWM's endpoint-pool decoupling pattern. Cross-environment generalization beyond manipulation tasks remains future work. The environments also do not cover locomotion, navigation, deformable objects, multi-object interaction, semantic planning, broad exploration data, mixed-quality behavior, or online collection.
...
We report seed-level variance and, for the revised core quantities, bootstrap confidence intervals. These intervals quantify pair-level uncertainty for the main pool-ranking and optimizer-attribution metrics, but the paper still relies primarily on pre-specified decision rules, replicated seeds, and cross-protocol consistency rather than broad hypothesis testing.
