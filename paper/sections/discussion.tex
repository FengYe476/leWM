...
\caption{Spatial diagnostics for optimizer and cross-environment analysis. \textbf{(a,b)}~CEM compresses the candidate pool into a tight latent-space cluster ($\mathrm{std}=7.9$), while MPPI with soft weighting preserves 3.7$\times$ more physical diversity ($\mathrm{std}=29.5$); both pools are shown for pair~61 under the same PCA projection, colored by physical cost $C_{\text{real\_state}}$. \textbf{(c,d)}~Goal-latent t-SNE maps for PushT (100 pairs) and OGBench-Cube (50 pairs), colored by $\Rpool(\Cmodel)$ with marker area proportional to pool success mass. Near-zero $\Rpool$ (yellow/red) dominates both environments, confirming cross-environment consistency of endpoint-planning decoupling.}
\label{fig:spatial_evidence}
\end{figure*}

The comparison supports a decomposed attribution, with substantial uncertainty in the optimizer-specific fraction. CEM hard truncation exacerbates the gap by compressing the candidate pool, but soft weighting does not eliminate it. Using $\Rendpoint=0.495$ at $m=192$ as the reference, MPPI retains only about 39\% of the endpoint ranking signal ($0.194/0.495$). The point estimate suggests that CEM-specific dynamics account for approximately 22\% of the endpoint-pool gap, $(0.194-0.115)/(0.495-0.115)$, though the paired-bootstrap 95\% confidence interval is wide ($[-15.7,48.1]\%$), reflecting substantial pair-level variability. The qualitative conclusion is therefore that soft weighting improves but does not eliminate the gap: the majority of the decoupling appears representation-driven, with the CEM-to-MPPI improvement in $\Rpool(\Cmodel)$ modest relative to the full endpoint-pool gap.
