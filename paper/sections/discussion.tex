...
\subsection{Cross-Architecture Check: DINO-WM}
\label{sec:dinowm}
...
\begin{table}[t]
\centering
\caption{Cross-model comparison of pool-level ranking on PushT. Both models exhibit near-zero $R_{\text{pool}}$ despite different architectures, encoders, and cost functions. DINO-WM results use 29 pairs $\times$ 1 seed (sanity-check scale), using 29 of the same 30 Track~A pairs after one pair was excluded due to scoring timeout.}
...
\scriptsize
\textit{Notes:} DINO-WM is a cross-architecture sanity check, not a full replication study. Its endpoint ranking is also near zero ($R_{\text{endpoint}} \approx 0.003$), so the comparison tests pool-level cost collapse and compression rather than the endpoint-passes-but-pool-fails pattern observed in LeWM.
\end{minipage}
\end{table}

DINO-WM validates that the three-level protocol reveals pool-level cost collapse in an architecturally distinct planner. However, since DINO-WM's endpoint ranking is itself near zero ($R_{\text{endpoint}} \approx 0.003$), this result demonstrates cross-architecture pool collapse and cost compression, not a clean replication of the endpoint-passes-but-pool-fails pattern observed in LeWM. A stronger cross-model test would require a planner whose endpoint ranking is clearly positive. DINO-WM's frozen DINOv2 features and patch-level MSE cost still produce a useful diagnostic signal: the terminal cost compresses in the CEM-converged region and loses local order preservation.

These results demonstrate that the three-level diagnostic protocol proposed in this paper applies directly to architecturally distinct latent planners and can distinguish global endpoint weakness from optimizer-local cost compression.
