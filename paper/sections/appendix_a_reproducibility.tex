...
main.tex} from \texttt{paper/}, using the TMLR template.
...
at \url{https://huggingface.co/collections/quentinll/lewm}. All numerical
results in this paper are reproducible from the frozen JSON artifacts listed in
Table~\ref{tab:appendix-commits}. The LeWM PushT checkpoint was downloaded from
\texttt{huggingface.co/quentinll/lewm-pusht} (file \texttt{weights.pt}); the
DINO-WM PushT checkpoint was downloaded from OSF (archive
\texttt{outputs.zip}, file
\texttt{outputs/pusht/checkpoints/model\_latest.pth}, DINO-WM commit
\texttt{0a9492f}). Exact HuggingFace snapshot revisions were not pinned at
download time; we recommend future audits pin the revision hash.
...
\input{sections/appendix_uncertainty_table.tex}
