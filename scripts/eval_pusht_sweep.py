#!/usr/bin/env python3
"""Sweep PushT goal offsets using the stable-worldmodel LeWM evaluation path."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a resumable PushT goal-offset sweep with the published LeWM "
            "stable-worldmodel planning loop."
        )
    )
    parser.add_argument(
        "--checkpoint-dir",
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Evaluation budget multiplier applied to each offset.",
    )
    ...
    parser.add_argument(
        "--warn-valid-start-points-below",
        type=int,
        default=10_000,
        help=(
            "Emit a warning when an offset leaves fewer than this many valid "
...
def get_budget(offset: int, multiplier: float, min_budget: int) -> int:
    return max(int(math.ceil(offset * multiplier)), min_budget)
...
def sample_eval_examples(
    ...
    *,
    ...
    # Match the existing baseline sampling range when possible so offset=25
    # remains directly comparable to prior runs.
...
def maybe_warn_for_offset(
    *,
    ...
    if analysis["valid_start_points"] < warn_threshold:
        print(
            f"WARNING: offset={offset} leaves only "
            f"{analysis['valid_start_points']} valid starting points "
    ...
    if analysis["eligible_episodes"] < num_eval:
        print(
            f"WARNING: offset={offset} leaves only "
            f"{analysis['eligible_episodes']} eligible episodes for "
            f"--num-eval={num_eval}."
        )

    if budget > analysis["max_episode_length"] * 2:
        print(
            f"WARNING: eval budget {budget} is large relative to the dataset's "
            f"max episode length {analysis['max_episode_length']}."
        )
...
def build_result_path(results_dir: Path, offset: int) -> Path:
    return results_dir / f"pusht_sweep_offset{offset}.json"
...
def evaluate_offset(
    *,
    ...
    maybe_warn_for_offset(
        offset=offset,
        budget=budget,
        num_eval=args.num_eval,
        analysis=analysis,
        warn_threshold=args.warn_valid_start_points_below,
    )
    ...
    for chunk_start in range(0, args.num_eval, args.num_envs):
        ...
        print(
            f"\n== Offset {offset}: episodes {chunk_start + 1}-{chunk_end} "
...
def main() -> int:
    ...
    budgets = {
        offset: get_budget(offset, args.budget_multiplier, min_budget)
    ...
    print("== PushT goal-offset sweep setup ==")
    ...
    print(f"budget_multiplier: {args.budget_multiplier}")
    ...
    for offset in args.offset_values:
        ...
        if analysis["max_episode_length"] <= offset:
            raise ValueError(
        ...
        print(
            ...
            f"{analysis['eligible_episodes']}/{index.total_episodes} eligible episodes, "
            f"budget={budget} =="
        )
        maybe_warn_for_offset(
            offset=offset,
